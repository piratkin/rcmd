...
using namespace std; //tell the compiler that you want to move everything from std to the global namespace
...
void ShowHelp()
{
   StdOutput( _T("Usage: xCmd.exe \\\\computer [options] command/exe arguments\n") );
   StdOutput( _T("\n") );
   ...
   StdOutput( _T("   xCmd.exe \\\\remote cmd       // Starts a \"telnet\" client\n") );
   StdOutput( _T("   xCmd.exe \\\\remote /nowait runme.exe\n") );
   StdOutput( _T("   xCmd.exe \\\\remote /user:administrator dir c:\\\n") );
   StdOutput( _T("   xCmd.exe \\\\remote /user:somebody /pwd:* /d:d:\\ test1.exe\n") );
   StdOutput( _T("   xCmd.exe \\\\remote /c /user:somebody /pwd:* /d:d:\\ test2.exe\n") );
   StdOutput( _T("\n") );
   StdOutput( _T("Notes:\n") );
...
BOOL PromptForPassword( LPTSTR lpszPwd )
{
   ...
   if ( EnableEcho( hInput, FALSE ) )
   {
      ...
      // Ignore ENTER (0x0D0A) 
      lpszPwd[max( (int)(dwRead-2), 0 )] = _T('\0');
...
BOOL InstallAndStartRemoteService()
{
   ...
   if ( hService == NULL )
      hService = ::CreateService(
         ...
         _T("%SystemRoot%\\system32\\")XCMDSVCEXE,
...
BOOL ConnectToRemoteService( DWORD dwRetry, DWORD dwRetryTimeOut )
{
   ...
   _stprintf( szPipeName, _T("%s\\pipe\\%s"), lpszMachine, XCMDCOMM );
...
BOOL FillMessage( xCmdMessage* pMsg )
{
